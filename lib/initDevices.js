...
function initDevices(jsonSystemState, I2C){
    ...
    // Restore devices to its last state.
    for(var dev in jsonSystemState){
        var devState = jsonSystemState[dev];
        // If device is connected to an i2c module:
        if(devState.type === 'i2c'){
            var switchState = devState.switchValue === 1 ? 'on' : 'off';
            changeRelayStateQueue.push({i2cBoardAddr: devState.i2cBoardAddr, relayID: devState.relayID, state: switchState}, function(err){
                if(err) return console.log(err);
                //console.log("New relay board state is: " + relayBoardState.toString(2));
            });
